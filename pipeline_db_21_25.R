...
CURRENT_SEASON <- 2025
CURRENT_WEEK <- 10  # Up to end of week 7
PREDICTION_WEEK <- 11  # Week we're predicting
...
env_file <- file.path(BASE_DIR, "config", ".env")
cat("  Looking for .env at:", env_file, "\n")
...
# Now use them
SPORTSDATA_API_KEY <- Sys.getenv("SPORTSDATA_API_KEY", unset = "795cb44abbcd438f8d5d276caaad4632")
WEATHER_API_KEY <- Sys.getenv("WEATHER_API_KEY", unset = "ee5e827a9793410492790704251311")
...
schedules <- nflreadr::load_schedules(CURRENT_SEASON) %>%
  ...
  select(game_id, season, week, gameday, gametime, weekday,
         ...
         location, roof, surface,   # <-- temp/wind removed
...
weather_file <- file.path(BASE_DIR, "weather_data.csv")
if (file.exists(weather_file)) {
  weather_existing <- tryCatch({
    read.csv(weather_file, stringsAsFactors = FALSE) %>%
      filter(week == PREDICTION_WEEK) %>%
      select(
        game_id,
        temp,
    ...
    cat("      ⚠️  Error loading weather_data.csv:", e$message, "\n")
    data.frame()
  })
  
  if (nrow(weather_existing) > 0) {
    # Join with schedules
    schedules <- schedules %>%
      left_join(weather_existing, by = "game_id") %>%
      mutate(
        # Calculate weather adjustment from impact score
        weather_point_adjustment = case_when(
          weather_impact_score >= 3 ~ -2.0,
          weather_impact_score == 2 ~ -1.5,
      ...
      select(-temp, -wind)  # Remove original columns
    
    weather_count <- sum(!is.na(schedules$weather_temp))
    cat("      ✓ Weather data loaded for", weather_count, "/", nrow(schedules), "games\n")
    
    # Show weather summary
    severe_weather <- schedules %>% 
      filter(weather_impact_score >= 2)
    if (nrow(severe_weather) > 0) {
      cat("      ⚠️  Games with significant weather impact:\n")
      for (i in 1:nrow(severe_weather)) {
        game <- severe_weather[i,]
        cat(sprintf("         • %s @ %s: %.0f°F, %.0f mph wind (impact: %d)\n",
    ...
    cat("      ⚠️  No weather data found for week", PREDICTION_WEEK, "\n")
    schedules <- schedules %>%
      mutate(
        ...
        weather_point_adjustment = 0
      )
  }
} else {
  cat("      ⚠️  weather_data.csv not found at:", weather_file, "\n")
  cat("      Expected location:", weather_file, "\n")
  schedules <- schedules %>%
    mutate(
      weather_temp = NA_real_,
      weather_wind = NA_real_,
...
# ============================================================================
# FETCH REAL WEATHER DATA (for upcoming games)
# ============================================================================

#cat("      Fetching real-time weather data...\n")

# Source the weather fetcher
#weather_script_path <- file.path(BASE_DIR, "weather_fetcher.R")
#if (file.exists(weather_script_path)) {
 # source(weather_script_path)
  
  # Fetch weather for all games
  #schedules <- tryCatch({
   # get_game_weather(schedules)
  #}, error = function(e) {
   # cat("      ⚠️  Weather fetch failed:", e$message, "\n")
    #cat("      Using fallback: No weather adjustments\n")
    #schedules %>%
     # mutate(
      #  weather_temp = NA_real_,
       # weather_wind = NA_real_,
...
#  })
#} else {
 # cat("      ⚠️  weather_fetcher.R not found at:", weather_script_path, "\n")
  #cat("      Using fallback: No weather adjustments\n")
  #schedules <- schedules %>%
   # mutate(
    #  weather_temp = NA_real_,
...
# ============================================================================
# FETCH REAL INJURY DATA (from SportsData.io)
# ============================================================================

cat("      Fetching real-time injury data from SportsData.io...\n")

# Source the injury fetcher
injury_script_path <- file.path(BASE_DIR, "sportsdata_injuries.R")
if (file.exists(injury_script_path)) {
  source(injury_script_path)
  
  # Fetch and process injuries
  injury_data <- tryCatch({
    fetch_and_process_injuries(season = CURRENT_SEASON, week = PREDICTION_WEEK)
  }, error = function(e) {
    cat("      ⚠️  Injury fetch failed:", e$message, "\n")
    cat("      Using fallback: No injury adjustments\n")
    list(
      detailed = data.frame(),
      summary = data.frame()
    )
  ...
  injuries <- injury_data$summary
  
  if (nrow(injuries) > 0) {
    cat("      ✓ Injury data loaded:", nrow(injuries), "teams with injury reports\n")
  }
  
} else {
  cat("      ⚠️  sportsdata_injuries.R not found at:", injury_script_path, "\n")
  cat("      Using fallback: No injury adjustments\n")
  injuries <- data.frame(
    team = character(), 
    total_injuries = numeric(), 
    players_out = numeric(), 
    players_doubtful = numeric(),
    players_questionable = numeric(), 
    key_injuries_out = numeric(),
    qb_injured = numeric(),
...
# Get most recent week's stats for predictions
# Use CURRENT_WEEK - 1 because we're predicting games that haven't been played yet
stats_week <- if(PREDICTION_WEEK > CURRENT_WEEK) CURRENT_WEEK else CURRENT_WEEK - 1

cat("      Using stats from week", stats_week, "for predictions\n")

latest_offense <- offense_rolling %>%
  filter(season == CURRENT_SEASON, week == stats_week)

latest_defense <- defense_rolling %>%
  filter(season == CURRENT_SEASON, week == stats_week)

# Verify we have data
if (nrow(latest_offense) == 0) {
  stop("ERROR: No offensive stats found for week ", stats_week)
}
if (nrow(latest_defense) == 0) {
  stop("ERROR: No defensive stats found for week ", stats_week)
}

cat("      ✓ Loaded stats:", nrow(latest_offense), "offensive teams,", 
    nrow(latest_defense), "defensive teams\n")
...
# Add injuries to matchups (already fetched above)
if (nrow(injuries) > 0) {
  matchups <- add_injuries_to_matchups(matchups, injuries)
  cat("      ✓ Injury data added to matchups\n")
} else {
  ...
  cat("      ⚠️  No injury data - using zeros\n")
}
...
predictions <- matchups %>%
  mutate(
    ...
    # Weather adjustment (use loaded weather data)
    weather_adj = case_when(
      !is.na(weather_point_adjustment) ~ weather_point_adjustment,
      is.na(weather_temp) ~ 0,
      weather_temp < 20 ~ -1.5,
      weather_temp < 32 ~ -0.75,
      weather_wind > 20 ~ -1.0,
      weather_wind > 15 ~ -0.5,
      TRUE ~ 0
    ),
    ...
    # Injury adjustment (using SportsData.io impact scores)
    injury_adj = case_when(
      !is.na(injury_advantage) ~ injury_advantage * 0.5,  # Use impact score
