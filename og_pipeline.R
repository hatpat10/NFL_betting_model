# ============================================================================
# NFL BETTING MODEL - ENHANCED MASTER PIPELINE (FIXED)
# ============================================================================
# CHANGES:
#   - BASE_DIR updated to C:/Users/Patsc/Documents/NFL_betting_model
#   - Week column added to all CSV outputs
...
cat("⚠️ CRITICAL REMINDER:\n")
cat("   This file has been updated with:\n")
cat("   1. Correct BASE_DIR path\n")
cat("   2. Week column added to CSV outputs\n")
cat("   Run this version for all future weeks!\n\n")
...
for (i in 1:nrow(schedule)) {
  ...
  if (!is.null(home_adv) && !is.null(away_adv)) {
    ...
    base_margin <- (home_adv$adjusted_epa_advantage - away_adv$adjusted_epa_advantage) * 15 + 2.5
    base_total <- 45 + (net_home_advantage * 10)
    
    # Get adjustment values
    weather_adj <- home_adv$weather_adjustment + away_adv$weather_adjustment
    injury_adj <- home_adv$injury_adjustment + away_adv$injury_adjustment
    rest_adj <- home_adv$rest_adjustment + away_adv$rest_adjustment
    
    # Calculate rest differential for additional margin adjustment
    rest_differential <- if(nrow(game_rest) > 0) {
      game_rest$home_days_rest - game_rest$away_days_rest
    } else {
      0
    }
    
    # Additional explicit rest adjustment for margin
    margin_rest_adj <- case_when(
      rest_differential >= 4 ~ 1.5,   # Home well-rested
      rest_differential <= -4 ~ -1.5, # Away well-rested
      rest_differential >= 2 ~ 0.5,
      rest_differential <= -2 ~ -0.5,
      TRUE ~ 0
    )
    
    # Additional explicit weather adjustment for total
    weather_impact_score <- if(nrow(game_weather) > 0) game_weather$weather_impact_score else 0
    total_weather_adj <- case_when(
      weather_impact_score >= 3 ~ -3.5,  # Severe conditions: reduce total
      weather_impact_score == 2 ~ -2.0,  # Moderate impact
      weather_impact_score == 1 ~ -1.0,  # Minor impact
      TRUE ~ 0
    )
    
    # Apply all adjustments
    final_margin <- base_margin + rest_adj + margin_rest_adj
    final_total <- base_total + weather_adj + total_weather_adj
    ...
    summary <- tibble(
      ...
      # FINAL PROJECTIONS (with all adjustments applied)
      projected_margin = final_margin,
      projected_total = final_total,
      ...
      # Adjustment contributions (for tracking)
      total_weather_adj = weather_adj + total_weather_adj,
      total_injury_adj = injury_adj,
      total_rest_adj = rest_adj + margin_rest_adj
    )
...
# Export enhanced data - ADD WEEK COLUMN FIRST

matchup_summary_df <- matchup_summary_df %>%
  mutate(week = WEEK) %>%
...
betting_recs <- matchup_summary_df %>%
  mutate(
    ...
    confidence = case_when(  # Fixed: = instead of <-
      abs(projected_margin) >= 10 ~ "VERY HIGH",  # Fixed: use projected_margin instead of edge
      abs(projected_margin) >= 7 ~ "HIGH",
      abs(projected_margin) >= 4 ~ "MEDIUM",
      abs(projected_margin) >= 2 ~ "LOW",
      TRUE ~ "VERY LOW"
    ),
    notes = case_when(
...
# Week column already added above, so no need to add again
write.csv(betting_recs, file.path(matchup_dir, "betting_recommendations_enhanced.csv"), row.names = FALSE)

cat("\n✓ Week column added to all CSV outputs!\n")
cat("  This is CRITICAL for backtesting to work.\n\n")
