...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    width = 300,
    sidebarMenu(
      menuItem("ðŸ“Š Dashboard Overview", tabName = "overview", icon = icon("dashboard")),
      menuItem("ðŸŽ¯ Top Picks", tabName = "picks", icon = icon("star")),
      menuItem("ðŸ“ˆ All Matchups", tabName = "matchups", icon = icon("table")),
      menuItem("ðŸ”¥ Power Rankings", tabName = "rankings", icon = icon("chart-bar")),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          box(
            title = "ðŸŽ¯ VALUE BETTING OPPORTUNITIES",
            status = "warning",
            solidHeader = TRUE,
            width = 12,
  ...
  output$top_picks_cards <- renderUI({
    ...
    cards <- lapply(1:nrow(picks), function(i) {
      ...
      if ("is_thursday" %in% names(pick) && !is.na(pick$is_thursday) && pick$is_thursday) {
        context <- paste0(context, "ðŸ“… Thursday Game ")
      }
    ...
    details <- lapply(1:min(5, nrow(data$matchups)), function(i) {
      ...
      div(
        ...
        if ("is_thursday" %in% names(game) && !is.na(game$is_thursday) && game$is_thursday) {
          p("ðŸ“… Thursday Night Game", style = "color: #f39c12;")
        } else {
          NULL
        }
      )
